...
export function randomDelay(min: number = 100, max: number = 1000, rng: () => number = Math.random, scheduler: (fn: (...args: any[]) => void, delay: number) => any = setTimeout): Promise<void> {
  const delay = Math.floor(rng() * (max - min + 1)) + min;
  return new Promise(resolve => scheduler(resolve, delay));
}

export function flakyApiCall(rng: () => number = Math.random, scheduler: (fn: (...args: any[]) => void, delay: number) => any = setTimeout): Promise<string> {
